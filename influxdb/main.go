...
func main() {
	...
	flag.StringVar(&(arguments.db), "db", "mydb", "DB to insert data")
	...
	flag.IntVar(&(arguments.filesNum), "numOfFiles", 10, "Number of files in dataDir ")
	...
	flag.IntVar(&(arguments.replicaN), "replicaN", 1, "Used to alter the default RP")
	flag.StringVar(&(arguments.writeConsistency), "consistency", "", "write consistency level (any,one,quorum,all)")
...
func writeData(arguments *ProArgs) {
	...
	log.Println("---- replicaN:", arguments.replicaN)
	log.Println("---- writeConsistency:", arguments.writeConsistency)
...
func writeDataImp(wInfo *WriteInfo, wg *sync.WaitGroup, arguments *ProArgs) {
	...
	_, err = queryDB(conn, fmt.Sprintf("ALTER RETENTION POLICY autogen ON %s DURATION INF REPLICATION %d", arguments.db, arguments.replicaN), arguments.db)
	if err != nil {
		log.Fatal(err)
	}
	...
	bp, err := client.NewBatchPoints(client.BatchPointsConfig{
		...
		Precision:        "ms",
		WriteConsistency: arguments.writeConsistency,
	})
	if err != nil {
		log.Fatal(err)
	}
...
func readData(arguments *ProArgs) {
	...
	for {
		...
		_, err = queryDBAsChunk(conn, sline, arguments.db)
		if err != nil {
			log.Fatal(err)
		}
...
func queryDBAsChunk(conn client.Client, cmd string, db string) (res []client.Result, err error) {
	query := client.Query{
		Command:   cmd,
		Database:  db,
		Chunked:   true,
	...
	var n int
	chunkedResponse, err := conn.QueryAsChunk(query)
	if err == nil {
		for {
			resp, err := chunkedResponse.NextResponse()
			if err == nil {
				res = append(res, resp.Results...)
				n++
				log.Printf("[%d] chunk got", n)
			} else if err == io.EOF {
				// done
				break
			} else {
